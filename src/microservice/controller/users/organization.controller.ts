...
export class UsersOrganizationController {
    ...
    // 添加班主任
    @ApiOperation({summary:"班级添加班主任接口", description:"班主任的添加"})
    @Patch("/classTeacherUpdate")
    async updateClassTeacher(@Body() updateClass : UpdateClassTeacherDto) {
      return await this.organizationService.updateClassTeacher(updateClass)
    }
  
    // 添加辅导员
    @ApiOperation({summary:"班级添加辅导员接口", description:"辅导员的添加"})
    @Patch("/classAssistantUpdate")
    async updateClassAssistant(@Body() updateClass : UpdateClassAssistantDto) {
      return await this.organizationService.updateClassAssistant(updateClass)
    }
