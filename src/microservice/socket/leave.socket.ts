...
// 请假模块socket
@WebSocketGateway(13004)
export class LeaveSocket implements OnGatewayInit {
    socketName = "请假-13004---";
    ...
    async handleConnection(client: Socket) {
        ...
        // 进行身份数据获取及验证
        const userData = await this.jwtAuth.authentication(token, "admin", [{ module: Admin.Leave, level: 1 }]);
        if (!userData.auth) {
    ...
    async afterInit(server: Server) {
        ...
        try {
            ...
            await this.channels.assertQueue(this.config.get('RabbitMQ_leave_socketQueueName'), { durable: true });
            // 队列监听
            await this.channels.consume(this.config.get('RabbitMQ_leave_socketQueueName'), (msg) => {
