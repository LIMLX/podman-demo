...
export class RepairsSocket implements OnGatewayInit {
    socketName = "报修-13005---";
    ...
    async handleConnection(client: Socket) {
        ...
        // 进行身份数据获取及验证
        const userData = await this.jwtAuth.authentication(token, "admin", [{ module: Admin.Repairs, level: 1 }]);
        if (!userData.auth) {
    ...
    async afterInit(server: Server) {
        ...
        try {
            ...
            await this.channels.assertQueue(this.config.get('RabbitMQ_repairs_socketQueueName'), { durable: true });
            // 队列监听
            await this.channels.consume(this.config.get('RabbitMQ_repairs_socketQueueName'), (msg) => {
